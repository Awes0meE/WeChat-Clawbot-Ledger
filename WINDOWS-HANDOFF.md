...
当前基线应为账本插件 215 项测试全部通过，stable-ID 插件 3 项测试全部通过；其中包含语义消费时间、可信时间上下文、日期只有/精确钟点边界、非 SGD 拒绝、跨插件实例确认、内外运行编号不同的微信回执替换、缺少外层账号元数据时的安全恢复、失败发送后释放预留、不同接收者隔离和候选不唯一时失败关闭。不要只依赖数量，任何失败都必须处理。
