...
同日后续排查确认，“中午吃饭7.1+2.5”曾因模型使用 `timeMode=received`、校验器却要求 explicit 而被拒绝；加法金额和疑问判断本身正常。按用户明确规则，“中午吃饭”“晚上吃饭”和“午饭”“晚饭”一样作为消费描述，没有另外写日期或具体钟点时统一按可信消息发送时间记账。日期、具体钟点、非 SGD 和真实疑问的边界继续保留。时间校验失败单独说明，不再误报成金额或语气不确定。当前正式 release 为 `1d561835030e880399252baad510cc58381e1eea`，完整回归 768/768、本机检查 14/14 通过；实际验收见下述系统检查记录。
