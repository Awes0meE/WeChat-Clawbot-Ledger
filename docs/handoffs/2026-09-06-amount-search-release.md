...
补充只读观察使用 OpenClaw 原生 SQLite，不生成或导出 JSONL：以真实 API 基线的本机毫秒时间为起点，窗口包含一个 session 的六个新增事件，无 transcript rewrite；其中有一条新用户消息、一次直接 `find_expenses` 调用和一个终态回复，终态文本的 SHA-256 与当前 formatter 一致。窗口首部有原生 `reset` 上下文边界，通用观察 helper 未完成完整父链与绑定关联复验；不把这项局部观察称为完整镜像链验证，也不将 helper 的拒绝解释成查询功能失败。原始内容仅在本机内存处理，未向终端、模型或 Git 输出交易正文、身份、SQL 数据行或日志原文。
