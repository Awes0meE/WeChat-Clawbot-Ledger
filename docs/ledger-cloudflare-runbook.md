...
本次双消息清理的证据范围：因实际 ASCII 测试标记大小写不同，原核验进程未能比较发消息前的三哈希。清理前重新建立快照，仅按经收据、实际备注、金额、ID 和完整内容哈希交叉核验的两个唯一目标删除；删除后 ID/标记均不存在、总数减 2，非目标交易与分类未变，账户状态仅发生准确的 0.03 SGD 余额返还。该结果证明删除前后的预期变化，不等于原始基线恢复；不得通过改备注、重放消息或恢复数据库来适配核验。后续验收仍须准备原始基线并如实记录实际比较范围。
