...
export type WeixinMsgContext = {
  Body: string;
  From: string;
  /** Stable provider sender id used by OpenClaw to populate trusted tool context. */
  SenderId: string;
  To: string;
  AccountId: string;
...
export function weixinMessageToMsgContext(
  ...
  const ctx: WeixinMsgContext = {
    ...
    // Preserve Tencent's durable message id so downstream integrations can
    // deduplicate a replay without collapsing two distinct same-text messages.
    MessageSid: msg.message_id != null ? String(msg.message_id) : generateMessageSid(),
