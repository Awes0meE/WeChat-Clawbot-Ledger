...
> **Clawbot 本地变体：** 本目录为基于腾讯 2.4.8 的 `@clawbot/openclaw-weixin-stable-id`，稳定消息 ID 与可信发送者映射见 [本地补丁说明](CLAWBOT-PATCH.md)。Clawbot 正式安装、更新与恢复遵循 [immutable release 运维手册](../../docs/ledger-cloudflare-runbook.md)。下文的上游安装、账号管理和卸载步骤作为参考保留，不是 Clawbot 的生产部署流程。

## 兼容性

| 本地插件版本 | 声明的最低 OpenClaw 版本 | 已验证的 Clawbot 宿主 |
| --- | --- | --- |
| `2.4.8-clawbot.1` | `>=2026.5.12` | `2026.8.2` |

表格记录本包的安装元数据与已验证宿主。保留的上游 `src/compat.ts` 运行时检查仍以
`2026.3.22` 为下限，宿主版本未提供或为 unknown 时跳过检查；它不强制执行安装元数据声明的
`2026.5.12` 下限。
