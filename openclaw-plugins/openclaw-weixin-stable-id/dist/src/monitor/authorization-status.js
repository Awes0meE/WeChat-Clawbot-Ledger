...
export function authorizationStatusForPoll(value) {
    const resp = value;
    if (resp?.localTransportTimeout === true)
        return { connected: false, lastError: "CLAWBOT_WEIXIN_POLL_TIMEOUT" };
    if (resp?.ret === -14 || resp?.errcode === -14)
        return { connected: false, lastError: "CLAWBOT_WEIXIN_REAUTHORIZATION_REQUIRED" };
    ...
    if (codes.some(code => code !== 0))
        return { connected: false, lastError: "CLAWBOT_WEIXIN_API_UNAVAILABLE" };
